...
export async function POST(req: NextRequest) {
  try {
    ...
    await discordService.send(
      `${user.merchant.name} created order trackingId: ${order.id}. payment link: ${res.uri}`
    );
    await smsService.send(
      order.phoneNumber,
      `StorefrontPay checkout url: ${res.uri}`
    );
...
export async function GET(req: NextRequest) {
  try {
    ...
    const limit = Number(req.nextUrl.searchParams.get("limit") || 20); //Leave this as 20 for now
    const startDate = new Date(req.nextUrl.searchParams.get("startDate") ?? 0);
    const endDate = new Date(
      req.nextUrl.searchParams.get("endDate") ?? Date.now()
    );
    ...
    if (!session.user.merchantId) {
      return NextResponse.json(
        { error: "Please process onboard first" },
        { status: 400 }
      );
    }
